...
describe('vendorExpenses - happy path (still passes after adding try/catch)', () => {
  test('aggregates totals and per-vendor breakdown from approved/synced vendor invoices', async () => {
    ...
    // Expected values per the GST-aware gross/net split merged in from main (see
    // dashboardController.js's grossAmountOf/netAmountOf): gross falls back to
    // extracted_total when total_including_gst isn't set (95.00 + 50.00 = 145.00);
    // net prefers verified_total where present, else gross - rebate (100.00 + 50.00 = 150.00).
    expect(data.summary).toMatchObject({ total_expenditure: '145.00', total_rebates_applied: '10.00', net_payable: '150.00', invoice_count: 2 })
    expect(data.by_vendor[0]).toMatchObject({ vendor_name: 'Acme Medical Supplies', total_expenditure: '145.00', total_rebates: '10.00', net_payable: '150.00', invoice_count: 2 })
  })
})
